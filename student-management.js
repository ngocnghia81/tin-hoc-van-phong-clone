document.addEventListener('DOMContentLoaded', function() {
    ...
    studentRows.forEach(row => {
        row.addEventListener('click', function() {
            ...
            const studentName = this.querySelector('td:nth-child(3)').textContent;
            const studentId = this.querySelector('td:nth-child(2)').textContent;
            const studentEmail = this.querySelector('td:nth-child(5)').textContent;
            const studentPhone = this.querySelector('td:nth-child(4)').textContent;
            const statusBadge = this.querySelector('.status-badge');
            const status = statusBadge.textContent;
            const statusClass = statusBadge.classList.contains('active') ? 'active' : 
                                statusBadge.classList.contains('inactive') ? 'inactive' : 'completed';
            
            // Update profile section
            document.querySelector('.student-name').textContent = studentName;
            document.querySelector('.student-id').textContent = `Mã HV: ${studentId}`;
            document.querySelector('.status-dot').className = `status-dot ${statusClass}`;
    ...
    if (studentSearch) {
        studentSearch.addEventListener('input', function() {
            ...
            studentRows.forEach(row => {
                ...
                const studentPhone = row.querySelector('td:nth-child(4)').textContent.toLowerCase();
    ...
    if (addStudentForm) {
        addStudentForm.addEventListener('submit', function(e) {
            ...
            const course = document.getElementById('newStudentCourse').options[document.getElementById('newStudentCourse').selectedIndex].text;
            ...
            const randomId = Math.floor(Math.random() * 10000000000) + 14900000000;
            ...
            newRow.addEventListener('click', function() {
                ...
                document.querySelector('.student-id').textContent = `Mã HV: ${randomId}`;
            ...
            const updatedStudentRows = document.querySelectorAll('.student-table tbody tr');
    ...
    if (updateBtn) {
        updateBtn.addEventListener('click', function() {
            ...
            const updatedStatus = document.getElementById('studentStatus').value;
            
            // Get the selected row
            const selectedRow = document.querySelector('.student-table tbody tr.selected');
            
            if (selectedRow) {
                // Update the row data
                selectedRow.querySelector('td:nth-child(3)').textContent = updatedName;
                selectedRow.querySelector('td:nth-child(4)').textContent = updatedPhone;
                selectedRow.querySelector('td:nth-child(5)').textContent = updatedEmail;
                ...
                // Update profile section
                document.querySelector('.student-name').textContent = updatedName;
                document.querySelector('.status-dot').className = `status-dot ${updatedStatus}`;
                document.querySelector('.status-text').textContent = statusBadge.textContent;
                
                // Show success message
                showNotification('Đã cập nhật thông tin học viên thành công!', 'success');
            } else {
                showNotification('Vui lòng chọn học viên để cập nhật!', 'warning');
            }
        });
    }
    ...
    if (confirmBtn) {
        confirmBtn.addEventListener('click', function() {
            ...
            if (selectedRow) {
                ...
                const studentName = selectedRow.querySelector('td:nth-child(3)').textContent;
                
                // Remove the row
                selectedRow.remove();
                
                // Reset the student details panel
                document.querySelector('.student-name').textContent = 'Chưa chọn học viên';
                ...
                // Show success message
                showNotification(`Đã xóa học viên ${studentName} thành công!`, 'success');
    ...
    if (printBtn) {
        printBtn.addEventListener('click', function() {
            ...
            if (selectedRow) {
                ...
                showNotification('Vui lòng chọn học viên để in thông tin!', 'warning');
            }
        });
    }
    ...
    if (historyBtn) {
        historyBtn.addEventListener('click', function() {
            ...
            if (selectedRow) {
                const studentName = selectedRow.querySelector('td:nth-child(3)').textContent;
                showNotification(`Đang tải lịch sử hoạt động của học viên ${studentName}...`, 'info');
                
                // In a real application, you would load the history data
                setTimeout(() => {
                    showNotification('Chức năng này sẽ được cập nhật trong phiên bản tới!', 'warning');
                }, 1500);
            } else {
                showNotification('Vui lòng chọn học viên để xem lịch sử!', 'warning');
            }
        });
    }
    ...
    if (uploadImageBtn) {
        uploadImageBtn.addEventListener('click', function() {
            ...
            showNotification('Chức năng tải ảnh sẽ được cập nhật trong phiên bản tới!', 'info');
        });
    }
